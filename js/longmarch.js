...
(function () {
  ...
  async function resolveVenueLinks() {
    ...
    if (!window.RedData) return;
    try {
      const venues = await window.RedData.loadAllVenues();
      STATIONS.forEach(s => {
        const siteName = VENUE_LOOKUP[s.venue] || s.venue;
        const v = window.RedData.findVenueByName(venues, siteName);
        if (v) {
  ...
  function buildContents(totalW, vh, perStationW, sidePad) {
    ...
    STATIONS.forEach((s, i) => {
      ...
      const photoHtml = photo
        ? `<div class="cz-note-photo"><img src="../images/longmarch/${photo}" alt="${s.name}实景" loading="lazy"></div>`
        : '';
  ...
  const CZ_CARD_SPIRITS = (window.RedCardGen && Array.isArray(window.RedCardGen.SPIRITS)) ? window.RedCardGen.SPIRITS : ['建党', '红船', '井冈山', '长征', '延安', '西柏坡', '抗战', '红岩', '红旗渠', '两弹一星', '苏区', '雷锋精神'];
  ...
  function buildCardModal() {
    ...
    czCardBgs.innerHTML = CZ_CARD_BGS.map((b, i) =>
      `<div class="cz-card-bg${i === 0 ? ' selected' : ''}" data-i="${i}" style="background-image:url('${bp}${b.src}')"><span>${b.label}</span></div>`
    ).join('');
  ...
  function openCardModal() {
    ...
    lockBody();
    if (czCardName) czCardName.focus();
  }
  ...
  function generateLongMarchCard() {
    ...
    if (!window.RedCardGen || typeof window.RedCardGen.renderCard !== 'function') {
      resetBtn();
      if (window.RedGuide && window.RedGuide.showToast) window.RedGuide.showToast('纪念卡模块未加载');
      return;
    }
    const img = new Image();
    img.onload = () => {
      try {
        _czCardDataUrl = window.RedCardGen.renderCard(img, spirit, name, '二万五千里 · 走完全程');
      } catch (e) {
        _czCardDataUrl = null;
        resetBtn();
        if (window.RedGuide && window.RedGuide.showToast) window.RedGuide.showToast('生成失败，请重试');
        return;
      }
      if (czCardPreview && czCardPreviewImg) {
        czCardPreviewImg.src = _czCardDataUrl;
        czCardPreview.style.display = 'block';
      }
      if (czCardSavehint) czCardSavehint.style.display = (('ontouchstart' in window) && window.innerWidth < 900) ? 'block' : 'none';
      resetBtn();
    };
    img.onerror = () => { _czCardDataUrl = null; resetBtn(); if (window.RedGuide && window.RedGuide.showToast) window.RedGuide.showToast('背景图加载失败'); };
    img.src = getBasePath() + bg.src;
  }
  function downloadLongMarchCard() {
    if (!_czCardDataUrl || !window.RedCardGen || typeof window.RedCardGen.dataUrlToBlob !== 'function') return;
    const blob = window.RedCardGen.dataUrlToBlob(_czCardDataUrl);
    if (!blob) return;
